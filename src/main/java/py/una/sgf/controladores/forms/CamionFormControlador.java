...
public class CamionFormControlador extends FormControladorAncestro<Camion> {
	...
	public String modal(Camion bean, ModelMap model) {
		...
		setBotonCrearHabilitado(false);
		setBotonGuardarHabilitado(false);
		setBotonCancelarHabilitado(false);
		index(bean, model);
		return urlModal;
	}
	 ...
	 * public String modalOp(@Valid final Camion bean, final BindingResult
	 * result, final ModelMap model,
	 *
	 * @RequestParam(Cons.Form.OPERACION_PARAM) String operacionParam) {
	 *
	 * boolean editandoPreviamente = false; if
	 * (operacionParam.equals(Cons.Form.GUARDAR_PARAM)) {
	 * setBotonElegirHabilitado(true);// onGuardarFallido: false
	 * editandoPreviamente = false; } else if
	 * (operacionParam.equals(Cons.Form.CANCELAR_PARAM)) { if
	 * (editandoPreviamente) { setBotonElegirHabilitado(true);
	 * editandoPreviamente = false; } } else if
	 * (operacionParam.equals(Cons.Form.MODIFICAR_REGISTRO)) {
	 * editandoPreviamente = true; } else { editandoPreviamente = false; }
	 * super.procesarForm(bean, result, model, operacionParam); return urlModal;
	 * }
	 */
