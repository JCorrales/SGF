...
import py.una.cnc.htroot.exception.ValidationException;
...
public abstract class SingletonDaoImpl<T extends Model> implements Dao<T> {
	...
	public T find(Long id) {
		...
		getLogger().info("Buscando registro. Entidad: {}, ID: {}", getEntityName(), id);
		...
		try {
			...
			getLogger().info("No se encontró registro. Entidad: {}, ID:{}", e, getEntityName(), id);
			return null;
		}
	}
	...
	public T findByCode(String code) throws DAOException {
		...
		return findEntityByCondition("WHERE " + getEntityName() + ".codigo = ?1 ", code);
	}
	...
	@Override
	@Transactional
	public T findEntityByCondition(String where, Object ... params) throws DAOException {
	...
	protected Query createQuery(String sql, String where, Object ... params) {
		...
		getLogger().debug("Ejecutando sentencia: {}, params: [{}]", sql, parameters);
	...
	public void validate(T object) {
		...
		for (ConstraintViolation<T> constraintViolation : constraintViolations) {
			 ...
			 */
			getErrorsMap().put(getEntityClass().getSimpleName() + "." + fieldName, constraintViolation.getMessage());
		}
		/*
		...
		if (getErrorsMap().size() > 0) {
			getLogger().info("Errores de validacion de {}: {}", getEntityName(), getErrorsMap());
